"""
Shared tracker PDF generation core.

This module contains reusable configuration, layout, drawing, PDF generation,
and CLI wiring used by both Goal Tracker and Project Tracker entry points.
"""
...
class GoalTrackerPDF:
    """Generates tracker PDFs with shared layout logic."""

    def __init__(
        self,
        config: GoalTrackerConfig,
        year: Optional[int] = None,
        tracker_type: str = "Goal",
    ):
        """
        ...
        Args:
            ...
            tracker_type: Label used in header text (for example Goal or Project)
        """
        if not isinstance(tracker_type, str):
            raise TypeError("tracker_type must be a string")

        normalized_tracker_type = tracker_type.strip()
        if not normalized_tracker_type:
            raise ValueError("tracker_type must be a non-empty string")
        ...
        self.tracker_type = normalized_tracker_type
    ...
    def draw_header(self, c: canvas.Canvas) -> None:
        """
        Render header section with title and tracker lines.
        ...
        # Allocate space: left 40% for title, right 60% for tracker target.
        left_w = grid_width * 0.4
        right_x = grid_left + left_w
        grid_width - left_w
        pad_x = 0.1 * inch

        # Left: title aligned to grid left.
        title_y = header_top_y - 0.10 * inch
        DrawingHelper.draw_text(
            c,
            f"{self.tracker_type} Tracker for {self.year}",
            grid_left + pad_x,
            title_y,
            font="Helvetica-Bold",
        ...
        # Right: tracker label and two lines, aligned to grid right.
        ...
        # Align the first input line with the label baseline.
        line1_y = goal_label_y
        # Second line below for additional detail with extra spacing between lines.
    ...
    def draw_weekly_column(self, c: canvas.Canvas) -> None:
        ...
        for row_index in range(1, self.layout.WEEKS_IN_YEAR + 1):
            ...
            week_text_width = c.stringWidth(
                week_text, self.font_config.family, self.font_config.week_number_size - 1
            )
            ...
            if self.layout.is_catch_up_week(row_index):
                ...
                if quarter == 4:
                    catch_up_text = "Close out Q4."
                else:
                    catch_up_text = f"Close out Q{quarter}. Plan next quarter."
...
def run_tracker_cli(tracker_type: str, description: str) -> None:
    """Run the shared CLI flow for a tracker variant."""
    ...
    try:
        ...
        if args.output:
            ...
            if tracker_type == "Project":
                output_filename = "project_tracker_template.pdf"
        ...
        # Generate PDF.
        pdf_generator = GoalTrackerPDF(config, year=year, tracker_type=tracker_type)
